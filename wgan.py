...
def next_real_batch(params):
    ...
    if 'lsun' not in params:
        # params['lsun'] = ore.Reader(ore.DATASET_LSUN_BEDROOM_TRAINING)
        params['lsun'] = ore.Reader(ore.DATASET_LSUN_BEDROOM_VALIDATION)

    reader = params['lsun']

    discriminator_batch_size = params.get('discriminator_batch_size', 128)

    raw_batch, _, _ = reader.next_batch(discriminator_batch_size)
